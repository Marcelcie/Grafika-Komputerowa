...
def compile_shaders():
    ...
    fragment_shader_source = """
        ...
        in vec4 vertex_color; // #nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn
        out vec4 color;
...
def startup():
    global rendering_program, vertex_array_object, vertex_buffer
...
def render(time):
    ...
    V_matrix = glm.lookAt(glm.vec3(0.0, 0.0, 1.0), glm.vec3(0.0, 0.0, 0.0), glm.vec3(0.0, 1.0, 0.0))
...
def main():
    ...
    window = glfwCreateWindow(600, 600, "LAB 7 - Cube Color Change", None, None)
    if not window:
        glfwTerminate()
        sys.exit(-1)
