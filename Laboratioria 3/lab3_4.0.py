...
C_TAB = np.zeros((N, N, 3), dtype=np.float32)
...
def calculate_egg_vertices():
    ...
    for i in range(N):
        ...
        for j in range(N):
            ...
            C_TAB[i, j] = [random.random(), random.random(), random.random()]
...
def render(time):
    ...
    for i in range(N - 1):
        for j in range(N - 1):
            glColor3f(C_TAB[i, j, 0], C_TAB[i, j, 1], C_TAB[i, j, 2])
            glVertex3f(V_TAB[i, j, 0], V_TAB[i, j, 1], V_TAB[i, j, 2])

            glColor3f(C_TAB[i + 1, j, 0], C_TAB[i + 1, j, 1], C_TAB[i + 1, j, 2])
            glVertex3f(V_TAB[i + 1, j, 0], V_TAB[i + 1, j, 1], V_TAB[i + 1, j, 2])

            glColor3f(C_TAB[i, j + 1, 0], C_TAB[i, j + 1, 1], C_TAB[i, j + 1, 2])
            glVertex3f(V_TAB[i, j + 1, 0], V_TAB[i, j + 1, 1], V_TAB[i, j + 1, 2])

            glColor3f(C_TAB[i + 1, j, 0], C_TAB[i + 1, j, 1], C_TAB[i + 1, j, 2])
            glVertex3f(V_TAB[i + 1, j, 0], V_TAB[i + 1, j, 1], V_TAB[i + 1, j, 2])

            glColor3f(C_TAB[i + 1, j + 1, 0], C_TAB[i + 1, j + 1, 1], C_TAB[i + 1, j + 1, 2])
...
def main():
    ...
    window = glfwCreateWindow(400, 400, "Lab 3 - Zadanie 4.0", None, None)
    if not window:
        glfwTerminate()
        sys.exit(-1)
